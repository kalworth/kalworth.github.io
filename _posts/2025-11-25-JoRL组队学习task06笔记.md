...
有点熟悉的概念，前文提到的经验回放，优先经验回放中对不同重要程度的样本进行采样。PPO的重要性采样是指什么呢以及PPO为什么要采用重要性采样？

- 为什么？
>策略梯度是一个会花很多时间来采样数据的算法，其大多数时间都在采样数据。智能体与环境交互以后，接下来就要更新参数。我们只能更新参数一次，然后就要重新采样数据， 才能再次更新参数。这显然是非常花时间的......

之前提到的策略梯度方法是同策略方法(on-policy)，什么意思呢，在策略梯度中，我们利用当前策略生成动作与环境交互得到轨迹，再利用这个轨迹去更新当前策略。当策略更新后，旧策略得到的那一条轨迹就没法用了，不仅浪费数据而且效率低下。为什么不能用，因为策略梯度是最大化当前策略下轨迹回报的期望.
...
不同的策略$\pi_{\theta}$和
$\pi_{\theta'}$下在相同的状态下会采取不同的动作，得到的轨迹就不相同，轨迹不相同，那么轨迹的概率分布
$p_\theta(\tau)$就不同,所以需要重新利用更新后的策略再生成轨迹进行训练。

PPO为了解决上面的问题，提出要充分利用旧策略的数据进行尽可能多的更新策略，以此提升效率。
