...
这里以我们的OJ举例，完成OJ上任务的基本步骤就是读懂题面，了解你要实现什么样的程序，接着写出相对应的程序，也就是在数学上实现这个问题的解，第一步对于我们而言都比较简单，而第二类错误，也是我们最容易犯，最需要注意，最需要debug的部分。
程序具体实现中的bug
...
6.*justMyCode*：true 只调试当前 py 文件，false 也调试引用的模块，当你需要调试模块时请确保将它修改为false
...
选择单步调试 程序将进入这行内的函数，展示函数内的运行步骤（这里没有函数概念的同学可以在后面的学习结束后再回过头继续看）
选择单步跳出 程序将跳出目前所在的函数
选择重启 程序将重新启动调试
选择终止 程序将立刻终止
...
### 如何调试非代码的错误？
在和在和组队学习的学习者们沟通的时候，我发现大多数同学一开始遇到的并不是代码bug，而是更多与手下代码无关的其他相关bug，例如环境变量没有设置好，环境无法正常启动，vscode不报错却无法正常运行等等非代码的错误，这些问题往往更复杂，更难处理，接下来我们将帮助你处理这些问题。  

在这之前，我们必须先普及一些基础概念。  

#### 什么是shell？
https://blog.csdn.net/weixin_34248849/article/details/88751641?spm=1001.2101.3001.6650.1&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-88751641-blog-129290846.235%5Ev32%5Epc_relevant_default_base3&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7ERate-1-88751641-blog-129290846.235%5Ev32%5Epc_relevant_default_base3&utm_relevant_index=2  

Shell是一种命令行解释器,它为用户提供了一个与系统交互的界面。Shell读取用户在终端输入的命令,并将其翻译成系统易于理解的语言,以便系统内核执行。常见的Shell有Bash、Zsh、Fish等。
#### 什么是环境变量？
绝大多数同学第一次接触环境变量都来自于环境配置中，安装python后需要将python，添加到环境变量中，说简单点就是将python 的路径保存在电脑中，并在终端注册，这样每次在终端内输入python ，计算机知道python是什么命令，并且能够知道它的路径在哪里，然后调用python
